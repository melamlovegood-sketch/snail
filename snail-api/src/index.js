...
async function handleIcal(token, url, env) {
  ...
  const tasksResp = await sbFetch(env,
    `/rest/v1/tasks?user_id=eq.${userId}&deleted_at=is.null${catFilter}&or=(deadline.not.is.null,start_time.not.is.null)&select=id,task_desc,deadline,start_time,task_date,dur_actual`
  );
